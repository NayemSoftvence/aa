...
exports.handler = async (event) => {
    ...
    try {
        ...
        // The receiver (callee) is the one accepting the call
        // We need to notify the caller
        if (call.calleeId !== uid) {
            return { statusCode: 403, headers: cors(), body: JSON.stringify({ error: 'Only callee can accept' }) };
        }

        // Get the caller's FCM tokens
        const callerDoc = await db.collection('users').doc(call.callerId).get();
        const tokensMap = callerDoc.exists ? (callerDoc.get('fcmTokens') || {}) : {};
        const tokens = Object.keys(tokensMap);
        if (!tokens.length) {
            console.log('No FCM tokens found for caller');
            return { statusCode: 200, headers: cors(), body: JSON.stringify({ status: 'no_tokens' }) };
        }

        // Send "call_accepted" notification to the caller
        ...
        console.log(`Call ${callId} accepted notification sent to caller ${call.callerId}`);
